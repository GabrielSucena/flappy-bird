...
function Barriers(barriersHeight, playWidth, barriersOpening, space, pointNotification) {
    ...
    const speedDisplacement = 3;
    this.animation = () => {
        this.pairs.forEach(pair => {
            pair.setX(pair.getX() - speedDisplacement);
            ...
            const crossedMiddle = pair.getX() + speedDisplacement >= middle &&
                pair.getX() < middle;

            if (crossedMiddle) pointNotification();
        });
    }
}
